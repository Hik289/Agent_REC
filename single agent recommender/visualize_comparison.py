...
# -*- coding: utf-8 -*-
"""
Visualize Comparison of Different Methods for Single Agent Recommender
...
def load_all_results(base_dir):
    ...
    if os.path.exists(output_old_dir):
        ltr_results_path = os.path.join(output_old_dir, 'ltr_test_results.json')
        tool_selection_path = os.path.join(output_old_dir, 'tool_selection_results.json')
        ...
        if os.path.exists(tool_selection_path):
            tool_data = load_json(tool_selection_path)
            total = 0
            correct_direct = 0
            correct_baseline = 0
            gt_in_candidates = 0

            for sel in tool_data.get('selections', []):
                if sel.get('ground_truth') and sel.get('selected_tools'):
                    total += 1
                    ground_truth = sel['ground_truth']
                    candidates = sel['selected_tools']

                    # Direct: top-1 from embedding retrieval
                    top1_tool = candidates[0]['tool_name']
                    if top1_tool == ground_truth:
                        correct_direct += 1
                    ...
                    for tool in candidates:
                        if tool['tool_name'] == ground_truth:
                            gt_in_candidates += 1
                            break
    ...
    if os.path.exists(output_dir):
        ltr_results_path = os.path.join(output_dir, 'ltr_test_results.json')
    ...
    if os.path.exists(output_merge_dir):
        ltr_results_path = os.path.join(output_merge_dir, 'ltr_test_results.json')
...
def visualize_test_results(results, figure_dir):
    ...
    ax.set_title('Single Agent Tool Selection - Method Comparison', fontsize=18, fontweight='bold',
                  pad=20, color='#2C3E50')
    ax.set_xticks(x)
...
def visualize_retrieval_vs_ranking(results, figure_dir):
    ...
    plt.tight_layout(pad=1.5)
    output_path = os.path.join(figure_dir, 'single_agent_retrieval_vs_ranking.png')
    safe_savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
...
def visualize_weights(base_dir, figure_dir):
    ...
    ax.set_title(f'Single Agent LTR - Feature Weight Distribution ({method_name})',
...
def visualize_ground_truth_retrieval(base_dir, figure_dir):
    ...
    for dir_name in ['output_merge', 'output_old']:
        selection_path = os.path.join(base_dir, dir_name, 'tool_selection_results.json')
...
def visualize_selection_stats(base_dir, figure_dir):
    """Visualize tool selection statistics"""
    selection_data = None
    for dir_name in ['output_merge', 'output_old']:
        selection_path = os.path.join(base_dir, dir_name, 'tool_selection_results.json')
        if os.path.exists(selection_path):
...
def print_summary(results):
    ...
    print("\n" + "="*80)
    print("Single Agent Learning-to-Rank - Method Comparison Summary")
    print("="*80)
...
def main():
    ...
    print("="*80)
    print("Single Agent Tool Selection - Method Comparison Visualization")
    print("="*80)
